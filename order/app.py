    ...
    try:
        ...
        if saga_result.get("success",False):
            ...
            for attempt in range(5):
                ...
                if user_id:
                    try:
                        ...
                        logging.error(f"[Order Orchestrator] Error checking payment status: {str(e)}")
            ...
            if payment_verified:
                ...
                logging.error(f"[Order Orchestrator] Payment status could not be verified for saga {saga_id}")
...
async def process_stock_completed(data, message):
    ...
    try:
        ...
        # Update saga state to indicate stock reservation is complete
        await update_saga_and_enqueue(
            saga_id=saga_id,
        ...
        # Update saga state to indicate payment initiation
        await update_saga_and_enqueue(
            saga_id=saga_id,
...
async def process_payment_completed(data, message):
    try:
        ...
        if order_released:
            ...
            # Even if lock release failed, try to publish completion notification
            completion_channel = f"saga_completion:{saga_id}"
            try:
                await db_master_saga.publish(completion_channel, json.dumps({"success": True}))
...
async def process_callback_messages(message):
    ...
    try:
        ...
        # Last resort - check if this is a reply message
